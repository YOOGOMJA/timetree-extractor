...
현재 단계는 **contract-first 1차 구현**이다. TimeTree Web 내부 data surface를 바로 product로 만들지 않고, synthetic fixture 기반 `RawTimeTreeEvent` validation과 `NormalizedCalendarEvent` 변환을 먼저 검증한다.
...
- 1차 구현은 schema validator, synthetic fixture, normalizer, warning/fail policy test로 제한한다.
- 실제 로그인된 page extractor, `ICS` writer, UI는 후속 gate 이후로 미룬다.
...
6. `docs/specs/timetree-extraction-contract.md` — raw extraction contract
7. `docs/specs/ics-normalization-contract.md` — normalization contract
8. `docs/architecture-notes.md` — 가능한 기술 구조와 보류된 선택지
...
현재 test는 Node built-in test runner만 사용하며, 추가 dependency는 없다.
