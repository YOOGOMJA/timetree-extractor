...
- Write human-facing repository documents in Korean by default.
- Keep widely used technical terms in English, for example `browser extension`, `content script`, `JSON`, `ICS`, `schema`, `DOM`, `API`, and `MVP`.
- Avoid abbreviations unless they are widely understood in the relevant technical context; prefer full terms over private or ambiguous shorthand.
